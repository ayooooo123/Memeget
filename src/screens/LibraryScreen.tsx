...
export function LibraryScreen() {
  ...
  // True while the user is actively dragging/flinging the grid. A full refresh
  // (re-fetch + re-merge the whole loaded span) mid-scroll re-lays-out the list
  ...
  const scheduleRefresh = useCallback(() => {
    ...
    refreshTimer.current = setTimeout(() => {
      ...
      // Defer a refresh that would land mid-scroll — flushed on scroll-end.
  ...
  // The grid reports when a drag/fling starts and settles. While scrolling we
  // (a) hold library refreshes (flushing any that queued on scroll-end) and
  // (b) stamp the interactive window so the heavy background loops (caption/DINO
  // backfills) stand down and don't compete for CPU mid-flick.
  const onScrollActiveChange = useCallback(
    (active: boolean) => {
