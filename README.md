...
| Meme format / character / emotion tags | **Zero-shot classification** against a curated prompt library (`src/memeLabels.ts`) — Pepe, Wojak, Doomer, Gigachad, Drake format, This Is Fine, etc. This is the editable "knowledge" layer. Each match carries a **calibrated confidence** and the classifier **abstains** rather than name a meme it doesn't recognize (`src/recognition.ts`) — most memes aren't a known template, and a forced guess would be handed to the AI describer as fact. Measured by `npm run recognition`. |
...
| Words *said* in the meme | Opt-in **audio analysis** (Settings): a native decoder (MediaCodec) pulls each video's audio track as normalized 16 kHz PCM and on-device **Whisper tiny English** transcribes it through react-native-executorch's supported native STT runner. The transcript shows in the viewer and is searchable — find a clip by the line you remember hearing. |
...
1. A *one-time* download of the CLIP model from Hugging Face on first launch
   ...
   model (Gemma 4 E2B / Whisper tiny English), cached the same way. After that, you can stay
...
src/audio.tsx           # Native Whisper transcription pass over video memes (opt-in)
src/audioCore.ts        # React-free audio core: model identity, PCM decode, cleanup, diagnostics
